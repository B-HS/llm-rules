...
| 문서                             | 범위                                  | 내용                                                                                                                                              |
| -------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| [ai-process.md](./ai-process.md) | **AI 작업 프로세스**                  | 응답 실행 계약, Codex·Claude Code workflow와 모델 배정, 상세 위임, 위험 비례 최소 검증, PROCESS.md·결과 분류                                      |
...
1. **시스템·개발자·실행환경 지시** (호스트가 강제하는 권한·안전·도구 계약 포함)
2. **사용자의 명시적 지시** (이번 대화에서 직접 말한 것)
3. **프로젝트 고유 룰** (그 레포의 `AGENTS.md` · `CLAUDE.md` · `docs/acknowledge` 의 합의)
4. **이 컨벤션 묶음** (index 요약과 상세 문서가 다르면 **상세 문서가 우선**)
5. **그 프로젝트의 기존 코드 패턴**
...
- **항상 간결하게, 존댓말로** 답한다. 첫 줄은 실행 행동이나 직접 답, 2단계 이상은 번호 목록, 마지막은 남은 2분 행동 하나 또는 관찰 가능한 완료 결과로 쓴다. 서론·반복 요약·마무리 인사는 쓰지 않는다.
- 현재 요청의 곁가지를 억제하고 매 응답에 상태를 한 줄로 표시한다. 표시상 상위 단계 하나만 진행 중으로 두되 독립 하위 작업은 병렬 실행한다. 시간은 관찰 근거가 있을 때만 쓰고, 오류는 원인·관찰값·해결을 사실적으로 제시하며 최종 목록은 그룹당 5개 이하로 묶는다.
- 설명 요청·파괴 작업·같은 가정의 3회 실패·실질적 모호성·답을 없애는 형식 충돌·실행환경 지시에는 필요한 예외를 적용하고 나머지 응답 형식은 유지한다.
- 새 도구 사용 작업마다 시작 전에 workflow 사용 여부를 한 번 묻고 답을 기다린다. 선택은 현재 작업에만 유효하며 새 세션·resume·clear·compact·handoff·`PROCESS.md`·prepare-new 재개에서는 다시 묻는다. 다른 에이전트의 chat/task/reopen/reset/summarize가 같은 의미면 같은 경계다. 사용자가 현재 요청에서 이미 선택했거나 workflow를 직접 호출했으면 중복 질문하지 않는다.
- workflow를 선택해도 Codex main 모델은 강제 덮어쓰지 않는다. 모호한 다단계·고위험 통합은 Sol high, 범위가 분명한 조율·분석은 Terra medium, 좁고 반복적인 작업은 Luna low/medium을 명시 배정하고, 복잡한 논리·보안·특히 어려운 판정에만 high/xhigh를 쓴다. Claude Code는 Fable high main, Sonnet high 일반 작업, Haiku xhigh 추가 판정을 사용한다. 선택하지 않으면 메인이 직접 수행하며 나머지 규칙은 유지한다.
- 메인은 요구사항·분해·통합·검증 근거·Git을 소유한다. 하위 지시에는 목표, 근거, 파일 소유권, 순서, 엣지 케이스, 최소 검증·재사용 조건, 보고 형식, 의존 관계를 빠짐없이 적는다.
...
- 확인이 필요하면 **현재 작업의 관련 결정만 한 번에** 모아 묻는다.
...
- 세션 시작 시 `docs/PROCESS.md` 를 먼저 읽고, 변경 위험을 직접 덮는 **최소 단일 검증**을 선택한다. 입력·환경이 같은 성공 결과는 재사용하고, 실패 영향 수정 뒤 관련 검사만 1회 재실행한다. 같은 가정이 3회 실패하면 중단해 질문하고, 지나치게 특수한 엣지는 `docs/quality-assurance`에 테스트 부채로 기록한다.
...
- 커밋은 **Conventional Commits v1.0.0** (`type(scope): 설명`, 기본 type 영어·설명 한국어). 메인 오케스트레이터는 위험 비례 검증을 마친 변경을 독립적으로 되돌릴 수 있는 논리 단위로 나눠 추가 승인 없이 자동 커밋·푸시한다. llm-rules는 일반 commit·push를 승인 대기시키거나 차단하는 프로젝트 수준 계층을 설치하지 않는다.
- 커밋 언어·스타일은 별도 지시가 없으면 **그 레포의 과거 커밋을 읽고 맞춘다.** 섞여 있으면 사용자에게 묻고 결정을 `docs/acknowledge` 에 기록해 이후 적용. ([git.md §1.1](./git.md))
- **AI 트레일러(Co-Authored-By 등) 금지**, `git add -A` 금지(선별 스테이징), force push 금지. 커밋 메시지 입력 방식은 제한하지 않으며 시스템·플랫폼·저장소 보호 정책은 우회하지 않는다. 에이전트는 `.env` 를 읽지도 쓰지도 않는다.
