...
- 사용자가 브랜치 생성·전환을 요청했거나 저장소 자체 정책이 요구할 때만 새 브랜치를 만든다. 그 외에는 현재 브랜치에서 일반 커밋·푸시를 진행한다.

---

## 6. 커밋 · 푸시 안전 규칙

- **메인 오케스트레이터는 구현과 위험 비례 검증이 끝나면 별도 요청이나 추가 승인을 기다리지 않고 커밋·푸시한다.** 사용자가 이번 작업에서 커밋 또는 푸시를 금지한 경우, Git 저장소가 아닌 경우, remote·인증이 없는 경우에는 실행하지 않고 이유를 보고한다.
- llm-rules가 설치하는 프로젝트 수준 규칙·hook·정책에는 일반 `git commit`·`git push`를 승인 대기시키거나 차단하는 계층을 두지 않는다. 시스템·개발자·플랫폼·저장소 보호 정책이 별도로 요구하는 승인을 우회한다고 약속하지 않는다.
...
- 각 논리 단위마다 [ai-process.md §8](./ai-process.md)의 최소 검증 근거를 확보한 뒤 `git status` → working tree diff → 선별 스테이징 → staged diff 확인 → 커밋 순서로 진행한다. 관련 입력·환경이 바뀌지 않은 성공 검증은 커밋 전에 반복하지 않는다. 모든 예정 커밋이 끝나면 현재 브랜치를 일반 push 한다.
...
- 커밋 메시지 입력 방식과 무관하게 이 문서의 형식과 AI 트레일러 금지를 지킨다. 프로젝트 수준 차단기에 의존하지 않고 메인 오케스트레이터가 커밋 전 staged diff와 최종 메시지를 확인한다.
