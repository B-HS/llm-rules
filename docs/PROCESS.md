...
- [x] **1. 기준선 보존·공식 기준 확인** — `v1` 브랜치의 `9adeed4`에 현재 상태를 스냅샷해 `origin/v1`에 보존하고 `codex/rules-v2`에서 작업 시작. OpenAI Docs의 SessionStart source와 Sol·Terra·Luna/effort 선택 기준 확인
...
- [x] **5. 회귀 검증** — `bun test` 4개·96 assertion, 양 hook shell·JSON·agent/config TOML·원격 설치기 embedded Python 문법, installer dry-run, 홈 화면 정적 build, `git diff --check` 통과. Git 전문·Rules·Claude permissions/workflow·코어 Git 섹션은 `v1`과 동일. raw SessionStart context는 Codex 26,817B→1,951B, Claude 28,134B→1,714B. `bun run typecheck`는 의존성 미설치로 `tsc: command not found`
...
- [x] **1. 충돌·자산 감사** — Terra 의미 감사와 Luna 자산 검색으로 병렬 실행 표시·중복 검증의 두 충돌을 분리하고, 사용자 확정에 따라 표시상 상위 단계 하나·실제 독립 작업 병렬 유지, 변경 없는 성공 검증만 재실행 금지, 실패 영향 수정 뒤 관련 검사 1회 허용으로 결정
- [x] **2. 공통·네이티브 규칙 수정** — 승인·차단 자산 제거, 위험비례 단일 검증·테스트 부채·저비용 xhigh 배정, 응답 작성 규칙을 공통 코어와 양쪽 workflow에 동기화
- [x] **3. 설치기·문서·회귀 테스트 정리** — 기존 managed Git guard를 설치 시 제거하고 사용자 자산을 보존하며 README·사이트·플랫폼 문서와 최소 회귀 테스트를 실제 동작에 맞춤
- [x] **4. 최소 검증·배포** — 변경별 검증 결과를 재사용해 installer 회귀 테스트·직접 파싱·정적 build를 한 번씩만 실행하고, typecheck는 직접 검증과 중복되어 생략. 글로벌 설치에서 guard 제거를 확인한 뒤 6개 논리 commit을 한 번 push하고 Release·Pages 결과를 확인

기준 문서: `docs/convention/ai-process.md` · `git.md` · `docs/agents-core.md` · Codex Subagent workflow Skill · Claude Code workflow Command.
...
- [x] **1. 공식 기준·현행 자산 병렬 감사** — Codex 공식 용어가 Subagent workflow·Subagent·Agent thread이며 custom agent의 `model`·`model_reasoning_effort` 설정을 지원함을 확인. Claude Code는 Fable·Sonnet 별칭과 subagent의 `model`·`effort`를 지원함을 확인. Terra·Luna 병렬 감사로 현재 guard는 승인·차단만 하고 실제 Git 실행·작업 분할·worker 배정은 없으며, `reinject-rules`와 `verify-on-stop`이 새 실행 계약과 중복됨을 확인
...
## 작업: README 설치 표·정적 문서 사이트 구조 개편 (완료)
...
- [x] **4. 검증·배포** — Prettier·typecheck·diff check 통과, lint·test 미설정 확인, `BASE_PATH=/llm-rules/ bun run build`로 23개 정적 경로 생성 및 데스크톱·390px 모바일 렌더 확인. `f607e79` push 후 Release·GitHub Pages Actions 성공
