...
`index.md` 자체는 규칙 원본이 아니라 다른 문서로의 라우팅·요약입니다. CC 에디션에서는 그 "요약(자주 어기는 핵심)"과 workflow 계약이 **세션 시작에 주입**되어 컨텍스트 드리프트를 막습니다.

| 규칙 (요약 항목) | → 메커니즘 | 등급 |
|---|---|---|
| 자주 어기는 핵심 규칙 요약 상시 인지 | `hook:session-context`(시작/재개 시 핵심 6줄 주입) | 결정론 졸업(주입은 기계적) |
| 긴 컨텍스트에서도 컨벤션 유지 | `hook:session-context`(시작/재개 시 workflow 계약 주입), `cmd:workflow` | prose 유지(작업 분해·통합은 메인 책임) |
...
| §4.1 매직넘버 금지(상수화) | `agent:convention-reviewer` + `cmd:workflow` | prose 유지 |
...
| §5.4 `any` 금지·`unknown` 경계한정 | `agent:type-utility-reviewer` + `cmd:workflow` | prose 유지 |
| §5.5 `enum` 금지 — `as const` + union | `agent:type-utility-reviewer` + `cmd:workflow` | prose 유지 |
...
| 규칙 | → 메커니즘 | 등급 |
|---|---|---|
| §4 `useCallback`/`useMemo` 금지(React Compiler 위임) | `hook:lint-edit (HARD)`(`use(Callback\|Memo)(` → block) | 결정론 졸업 |
...
| 규칙 | → 메커니즘 | 등급 |
|---|---|---|
| §2 의존성 방향(app→pages→widgets→features→entities→shared, 역방향 금지) | `agent:fsd-dependency-reviewer`, `cmd:audit-fsd`, `cmd:workflow` | prose 유지(import 그래프 판단은 에이전트) |
...
| §13 테스트(`bun:test`, 한국어 설명) | `permission`(`bun test` allow), `agent:verification-worker`, `cmd:workflow` | prose 유지(테스트 범위는 작업 계약) |
...
- **컨벤션 상시 인지**(index 요약, ai-process §10) → `hook:session-context`.
