...
2. Fable high 메인은 요구사항·작업 분해·의존 관계·통합·최종 판정·Git만 소유한다. Sonnet high 서브에이전트에는 정확한 목표/DoD, 실제 파일·심볼 근거, 파일 소유권·비목표, 적용 규칙·커맨드, 구현 순서, edge case·금지 우회, 최소 검증 명령·기존 성공 결과·합격 기준, 보고 형식, Git 금지, 의존·대기 관계를 모두 전달한다.
3. 독립적인 읽기·구현·검증 작업은 병렬로 위임하고, 같은 파일을 쓰거나 선행 결과가 필요한 작업은 직렬로 진행한다. 주 검증 뒤 실질적인 애매성만 Haiku xhigh 작업자에게 최소 검사 하나로 맡긴다. 지나치게 특수하면 실행하지 않고 QA 테스트 부채로 남긴다.
