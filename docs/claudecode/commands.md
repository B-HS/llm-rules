...
| `/llm-rules:workflow` | 오케스트레이션 | Fable main, Sonnet high 작업자, Haiku xhigh 보조 검증으로 도구 사용 작업을 분해·위임·통합 | (선택) 작업 설명 또는 현재 사용자 요청 |
...
- **인자**: (선택) 검증 범위.
- **연계**: workflow의 `verification-worker`가 같은 검증 계약을 수행하고, 메인은 명령을 다시 실행하지 않고 실제 출력을 최종 판정에 재사용합니다.
