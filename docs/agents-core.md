...
- **세션 시작 시퀀스**: ① 베이스 룰 확인 → ② `docs/PROCESS.md` 읽기(진행 중 체크리스트 파악) → ③ 필요 시 `docs/memory` · `docs/acknowledge` → ④ workflow 사용 여부 1회 확인 후 답을 받고 실행.
- **새 도구 사용 작업마다 시작 전에 workflow 사용 여부를 한 번 묻는다.** 선택은 현재 작업에만 유효하며, 새 세션·resume·clear·compact·handoff·`PROCESS.md` 재개에서는 이전 선택을 승계하지 않고 다시 묻는다. 현재 요청이 이미 선택했거나 workflow를 직접 호출했으면 중복 질문하지 않는다.
...
- **메인 오케스트레이터**: Codex `gpt-5.6-sol` reasoning `high`, Claude Code `fable` effort `high`. 요구사항·분해·의존성·파일 소유권·결과 통합·검증 근거 판단·Git을 전담한다.
- **하위 작업**: Codex는 복합 구현·리서치·의미 검증에 `gpt-5.6-terra` `high`, 좁고 반복적인 조사·기계 검증에 `gpt-5.6-luna` `high`; Claude Code는 구현·리서치·검증에 `sonnet` `high`. 필요성이 애매한 추가 검사는 Codex `gpt-5.6-luna` `xhigh`, Claude Code `haiku` `xhigh`가 먼저 가치와 최소 범위를 판단한다. 하위 에이전트는 commit·push 금지.
