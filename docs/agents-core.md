...
- 규칙 충돌 사다리: **시스템·개발자·실행환경 > 사용자 명시 지시 > 프로젝트 고유 룰 > 이 컨벤션 > 기존 코드 패턴.** 컨벤션과 다른 선택을 했으면 사용자에게 알린다.
...
- **하위 에이전트의 commit·push / AI 트레일러(`Co-Authored-By` 등) / `git add -A` / force push / 검증 전 자동 커밋.** 메인 오케스트레이터는 git §6에 따라 위험 비례 검증 후 추가 승인 없이 자동 커밋·푸시한다.
- **검증 없이 "통과했다" 보고.**
...
- 응답은 **한국어 + 존댓말, 간결하게.** 첫 줄에 실행 행동이나 직접 답을 쓰고, 2단계 이상은 행동 하나씩 번호를 매긴다. 남은 작업이 있으면 2분 안의 다음 행동 하나로, 없으면 관찰 가능한 완료 결과로 끝낸다.
- 현재 요청의 곁가지를 억제하고 매 응답에 현재 상태를 한 줄로 표시한다. 표시상 상위 단계 하나만 진행 중으로 두되 독립 하위 작업의 실제 병렬 실행은 유지한다. 관찰 근거가 있을 때만 구체 시간을 쓰고, 없으면 다음 상태 조건을 쓴다.
- 오류는 원인·관찰값·해결을 사실적으로 제시하고, 완료 결과는 명령·경로·실행 결과로 보인다. 최종 목록은 관련 항목을 그룹당 5개 이하로 묶으며 서론·반복 요약·마무리 인사를 쓰지 않는다.
- 설명 요청, 파괴적 작업 사전 확인, 같은 가정의 3회 실패 중단, 실질적 모호성 질문, 답을 없애는 형식 충돌, 실행환경 우선 상황에서는 필요한 만큼 응답 형식을 조정한다. 전송 전 계획 예고·추가 도움 질문·곁가지 연결어·정보 없는 불확실성·관용 표현을 제거한다.
- **세션 시작 시퀀스**: ① 베이스 룰 확인 → ② 주입된 `docs/PROCESS.md` 활성 작업 확인(없거나 세부가 필요할 때만 파일 읽기) → ③ 필요 시 `docs/memory` · `docs/acknowledge` → ④ workflow 사용 여부 1회 확인 후 답을 받고 실행.
- **새 도구 사용 작업마다 시작 전에 workflow 사용 여부를 한 번 묻는다.** 선택은 현재 작업에만 유효하며, 새 세션·resume·clear·compact·handoff·`PROCESS.md`·prepare-new 재개에서는 이전 선택을 승계하지 않고 다시 묻는다. 현재 요청이 이미 선택했거나 workflow를 직접 호출했으면 중복 질문하지 않는다. 다른 에이전트의 새 chat/task, reopen/reset/summarize 등 이름이 달라도 의미가 같으면 같은 경계다.
- **사용**이면 Codex Subagent workflow 또는 Claude Code subagent workflow를 적용한다. **사용하지 않음**이면 메인이 직접 수행하되 나머지 규칙은 유지한다. 답을 받기 전에는 자동 주입 컨텍스트 확인 외의 실행 도구를 사용하지 않는다.
- **메인 오케스트레이터**: Codex main 모델을 설치 설정으로 고정하지 않는다. 모호한 다단계·고위험 통합은 `gpt-5.6-sol` `high`, 범위가 분명한 일반 조율은 `gpt-5.6-terra` `medium`을 선택한다. Claude Code는 `fable` `high`다. 메인은 요구사항·분해·의존성·파일 소유권·결과 통합·검증 근거 판단·Git을 전담한다.
- **하위 작업**: Codex는 일반 분석·구현에 `gpt-5.6-terra` `medium`, 복잡한 구현·보안 판단에 Terra `high`, 좁고 반복적인 조사·기계 검증에 `gpt-5.6-luna` `low|medium`을 명시 배정한다. 어려운 추가 판정만 Luna `high|xhigh`로 올린다. Claude Code는 구현·리서치·검증에 `sonnet` `high`, 애매한 추가 검사에 `haiku` `xhigh`를 사용한다. 하위 에이전트는 commit·push 금지.
- 독립 작업은 병렬, 같은 파일·선행 의존 작업은 직렬로 배정한다. 직렬 작업도 최소 하나의 경계가 분명한 하위 작업을 거친다. 메인은 하위 보고의 실제 diff·파일·명령 출력을 점검하되, 입력·환경이 같은 성공 명령은 재실행하지 않는다.
...
- 현재 작업의 관련 질문만 **결정 1개면 1줄 객관식, 2개 이상이면 한 묶음**(추천안 먼저, "전부 추천안대로" 답변 가능하게)으로 제시한다.
...
- **위험 비례 최소 검증**: 변경 위험을 직접 덮는 단일 검사가 기본이며, 서로 독립된 위험이 있을 때만 추가한다. 입력·환경이 같은 성공 결과는 통합·커밋 전에 재사용한다. 실패 영향 수정 뒤 관련 검사만 1회 재실행하고, 같은 가정이 3회 실패하면 중단해 진단 질문 하나만 한다. 지나치게 특수한 엣지는 `docs/quality-assurance`에 재현 조건·생략 이유·위험·실행 시점을 테스트 부채로 기록한다.
...
- 브랜치는 필요할 때 `<type>/<요약>` kebab-case로 만들며, 별도 요청이나 저장소 정책이 없으면 현재 브랜치에서 진행한다.
- **메인 오케스트레이터는 구현·위험 비례 검증 완료 후 추가 승인 없이 자동 커밋·푸시**한다. llm-rules가 설치하는 프로젝트 수준 규칙·hook·정책은 일반 commit·push를 승인 대기시키거나 차단하지 않는다. 시스템·플랫폼·저장소 보호 정책은 우회하지 않는다. 사용자가 금지했거나 Git/remote/인증이 없으면 실행하지 않고 보고하며, 하위 에이전트는 Git 작업을 하지 않는다.
- 독립적으로 되돌릴 수 있는 동작·규칙·배포 단위로 커밋을 나눈다. 관련 구현·테스트·문서는 함께 두고, 동작하지 않는 중간 상태는 커밋하지 않는다. **한 커밋분만 선별 스테이징**(`git add -A`/`.` 금지) → `git status`·working/staged diff 확인 → commit 순서로 진행하고, 예정된 커밋이 끝나면 일반 push 한다.
- **author 는 사용자 단독** — `Co-Authored-By`·`Generated with`·`Codex-Session:`·`Claude-Session:` 등 세션 링크와 AI 서명·트레일러 금지.
- 커밋 메시지 입력 방식과 무관하게 staged diff와 최종 메시지를 확인한다. `--force-with-lease`를 포함한 **모든 force push를 금지**하고, 이력 재작성은 사용자와 별도 방향을 정한다.
