...
| Hook | 이벤트 | 동작 |
|---|---|---|
| `guard-commit.sh` | `PreToolUse(Bash)` | main·master 직접 커밋, AI 트레일러, 금지 파일 staging, Conventional Commit 위반 차단. 합의 저장소의 자동 커밋 승인 |
| `guard-push.sh` | `PreToolUse(Bash)` | 위치와 무관하게 `--force`·`-f` 차단. 합의 저장소의 일반 push 승인 |
...
| `verify-on-stop.sh` | `Stop` | 코드 변경이 있으면 프로젝트 typecheck 실행, 실패 시 Codex가 작업을 계속하도록 요구 |
| `session-context.sh` | `SessionStart` | 컨벤션 요약, 작업 개시 프로토콜, `docs/PROCESS.md` 앞부분 주입 |
| `reinject-rules.sh` | `UserPromptSubmit` | 긴 세션의 규칙 드리프트를 줄이는 짧은 리마인더 주입 |
...
여러 matching hook은 동시에 시작될 수 있으므로 각 Git hook이 입력 명령을 다시 검사해 자신의 대상이 아니면 즉시 종료합니다. Hook은 완전한 보안 경계가 아니라 Rules, sandbox, AGENTS 지침과 함께 쓰는 guardrail입니다.
