...
- **`git push --force`(`-f`) 를 사용하지 않는다.** 히스토리 재작성(rebase·amend 후 push 포함)이 필요하면 사용자에게 묻는다.

### 6.1 AI 에이전트의 커밋 (모든 에이전트 공통)

- **author 는 사용자 단독이다.** 커밋 메시지에 `Co-Authored-By`, `Generated with ...`, 🤖 등 **AI 도구의 서명·트레일러·이모지를 넣지 않는다.**
