...
- **메인 오케스트레이터는 구현과 검증이 끝나면 별도 요청을 기다리지 않고 커밋·푸시한다.** 사용자가 이번 작업에서 커밋 또는 푸시를 금지한 경우, Git 저장소가 아닌 경우, remote·인증이 없는 경우에는 실행하지 않고 이유를 보고한다.
- 하위 에이전트는 커밋·푸시하지 않는다. Git 이력과 원격 상태의 최종 책임은 [ai-process.md](./ai-process.md) §1.2의 메인 오케스트레이터에만 있다.
- 커밋은 **독립적으로 되돌릴 수 있는 논리적 단위 1개**로 묶는다. 파일 종류가 아니라 사용자에게 보이는 동작·규칙·배포 단위를 기준으로 나누며, 관련 구현·테스트·문서는 같은 커밋에 둔다. 무관한 변경을 한 커밋에 섞거나 동작하지 않는 중간 상태를 커밋하지 않는다. (→ [ai-process.md](./ai-process.md) §6.8 최소 변경)
...
- 각 논리 단위마다 관련 검증을 통과한 뒤 `git status` → working tree diff → 선별 스테이징 → staged diff 확인 → 커밋 순서로 진행한다. 모든 예정 커밋과 최종 검증이 끝나면 현재 브랜치를 일반 push 한다.
- **모든 형태의 force push를 사용하지 않는다.** `--force`, `-f`, `--force-with-lease`, `--force-if-includes`를 모두 금지한다. 히스토리 재작성이 필요하면 자동으로 진행하지 않고 사용자와 별도 해결 방향을 정한다.

### 6.1 AI 에이전트의 커밋 (모든 에이전트 공통)

- **author 는 사용자 단독이다.** 커밋 메시지에 `Co-Authored-By`, `Generated with ...`, `Codex-Session:`, `Claude-Session:` 같은 AI 세션 링크 등 **AI 도구의 서명·트레일러·이모지를 넣지 않는다.**
- 커밋 메시지는 이 문서의 형식(§1~§4)을 그대로 따른다. 에이전트가 임의 형식을 만들지 않는다.
- 자동 커밋도 inline `-m` 메시지를 사용해 guard가 전체 메시지를 검사할 수 있게 한다. `-F`, `--file`, editor 입력처럼 사전 검사가 불가능한 메시지 경로는 사용하지 않는다.
