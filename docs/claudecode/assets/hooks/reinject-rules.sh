...
ctx="[리마인더] arrow-fn only · 주석 금지(JSDoc 만) · any/enum 금지(unknown 은 경계에서만+즉시 좁힘) · 매직넘버·이모지 금지 · useCallback/useMemo 금지(useEffect 는 외부 동기화만) · named export · FSD 의존 방향 위→아래(barrel 금지) · Conventional Commits · Co-Authored-By 금지 · 요청 전 commit/push 금지 · git add -A/force push 금지 · 시크릿 하드코딩·.env 접근 금지 · 종료 전 검증(typecheck→lint→test) · 모호하면 1줄 객관식 질문."
